...
def make_gradient_engine(func, tensor, num_perturbations, device):
    if tensor.dim() == 1:
        return Grayscale_Engine(func, tensor, num_perturbations, device)
    elif tensor.dim() == 3:
        return RGB_Engine(func, tensor, num_perturbations, device)
    else:
...
class Grayscale_Engine(Gradient_Engine):
    def __init__(self, func, tensor, num_perturbations, device):    #device parameter needs to be the same as the tensor and the func's respective devices
        ...
        super().__init__(func, tensor, num_perturbations, device)
