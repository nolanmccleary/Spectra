...
def generate_phash_batched(batched_tensor, dct_dim=8):
    if batched_tensor.dim() == 3:
        batched_tensor = batched_tensor.unsqueeze(0)
    return torch.stack([_generate_phash(v, dct_dim) for v in batched_tensor], dim=0)

#[C, H, W] -> [dct_dim * dct_dim]
def _generate_phash(tensor, dct_dim):
